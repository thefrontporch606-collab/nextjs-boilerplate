...
export default function HometownHeroesPage() {
  ...
  const HeroCard = ({ hero }: { hero: Veteran }) => (
    <article id={hero.id} className={`hero-card ${hero.fallen ? "fallen" : ""} ${highlighted === hero.id ? "highlighted" : ""}`}>
      <button className="card-main" type="button" onClick={() => setSelected(hero)} aria-label={`View full profile for ${hero.rank} ${hero.name}`}>
        <div className="photo-wrap">
          {hero.image ? <img src={hero.image} alt={`${hero.rank} ${hero.name}`} /> : <div className="photo-placeholder">Photo</div>}
        </div>
        <div className="card-copy">
          <p className="rank">{hero.rank}</p>
          <h3>{hero.name}</h3>
          <p className="city">{hero.city}</p>
          <span className="expand-label">Tap to view</span>
        </div>
      </button>
    </article>
  );
  ...
  return (
    <main className="page-shell">
      ...
      <style jsx>{`
        .page-shell{min-height:100vh;background:#f2f3f5;color:#17212b}.masthead{text-align:center;padding:28px 18px 34px;background:linear-gradient(#fff 0%,#d8aaa3 32%,#8d3940 58%,#17375d 100%);border-bottom:5px solid #d0b44a}.logo{width:min(430px,90vw);height:auto}.salutes{margin:0;color:#fff;font-size:clamp(22px,4vw,36px);font-weight:800;text-shadow:0 2px 8px #000}.title-art{width:min(760px,94vw);margin:0 auto}.branches{width:min(660px,90vw);margin:14px auto 22px}.primary-button,.search-form button{border:0;border-radius:999px;padding:13px 22px;background:#a51e2c;color:#fff;font-weight:800;cursor:pointer}.search-section{padding:24px 16px;background:#102f4e;color:#fff}.search-inner{max-width:760px;margin:auto;text-align:center}.search-inner h1{margin:0 0 6px;font-size:clamp(22px,4vw,32px)}.search-inner p{margin:0 0 14px;color:#dbe6ef}.search-form{display:grid;grid-template-columns:1fr auto;gap:8px}.search-form input{min-width:0;padding:14px 16px;border:2px solid transparent;border-radius:12px;font-size:16px}.search-form input:focus{outline:none;border-color:#d0b44a}.results{display:grid;gap:6px;margin-top:8px;padding:8px;background:#fff;border-radius:12px;color:#17212b;text-align:left}.results button{padding:11px;border:0;border-radius:8px;background:#eef3f8;text-align:left;font-weight:700;cursor:pointer}.results p{margin:4px;color:#5b6772}.wall-section{padding:34px 12px}.wall-section h2{max-width:1050px;margin:0 auto 24px;text-align:center;font-family:Georgia,serif;font-size:clamp(17px,3vw,28px)}.veterans-section{background:#e6e9ed}.grid{max-width:1180px;margin:auto;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:clamp(6px,1.5vw,18px);align-items:stretch}.hero-card{min-width:0;height:100%;scroll-margin-top:120px;border-radius:clamp(8px,1.5vw,18px);background:#fff;box-shadow:0 7px 20px rgba(15,35,55,.12);overflow:hidden;border:2px solid transparent;transition:.25s}.hero-card.fallen{border-color:#c3a445}.hero-card.highlighted{border-color:#c51f36;box-shadow:0 0 0 4px rgba(197,31,54,.2),0 12px 30px rgba(15,35,55,.2);transform:translateY(-3px)}.card-main{display:flex;width:100%;height:100%;min-width:0;flex-direction:column;padding:0;border:0;background:transparent;cursor:pointer;color:inherit}.photo-wrap{aspect-ratio:1/1;background:#d9dee4;overflow:hidden}.photo-wrap img{width:100%;height:100%;object-fit:cover;object-position:center top}.photo-placeholder{height:100%;display:grid;place-items:center;color:#5b6772;font-size:.7rem}.card-copy{display:flex;min-height:6.8rem;flex:1;flex-direction:column;align-items:center;padding:clamp(7px,1.4vw,16px) clamp(5px,1vw,12px);text-align:center}.rank{margin:0 0 2px;font-weight:900;letter-spacing:.06em;color:#8e1f2e;font-size:clamp(.56rem,1.5vw,.88rem)}.card-copy h3{margin:0;font-size:clamp(.68rem,2vw,1.2rem);line-height:1.15;overflow-wrap:anywhere}.city{margin:5px 0 0;font-size:clamp(.55rem,1.35vw,.82rem);color:#526174}.expand-label{margin-top:auto;padding-top:6px;color:#174f79;font-size:clamp(.5rem,1.2vw,.72rem);font-weight:800}.modal-backdrop{position:fixed;inset:0;z-index:2000;display:grid;place-items:center;padding:18px;background:rgba(5,16,27,.78)}.modal{position:relative;width:min(620px,100%);max-height:92vh;overflow:auto;padding:26px;border-radius:18px;background:#fff;text-align:center}.close{position:absolute;right:12px;top:8px;border:0;background:transparent;font-size:34px;cursor:pointer}.modal-photo{width:min(280px,100%);aspect-ratio:4/5;object-fit:cover;object-position:center top;border-radius:14px;margin:0 auto 18px}.modal h2{margin:2px 0 12px}.modal p{margin:6px}.actions{display:grid;grid-template-columns:1fr 1fr;gap:9px;margin-top:20px}.actions button,.actions a{display:flex;align-items:center;justify-content:center;min-height:44px;padding:10px;border:1px solid #cbd5df;border-radius:10px;background:#f4f7fa;color:#17375d;text-decoration:none;font-weight:700;cursor:pointer}.submission-form{display:grid;gap:10px;margin-top:18px}.submission-form input{padding:13px;border:1px solid #cbd5df;border-radius:9px;font-size:16px}@media(max-width:600px){.search-form{grid-template-columns:1fr}.actions{grid-template-columns:1fr}.masthead{padding-top:18px}.wall-section{padding:26px 6px}.wall-section h2{padding-inline:6px}.card-copy{min-height:6.2rem}}@media print{.masthead,.search-section,.wall-section,.close,.actions{display:none}.modal-backdrop{position:static;background:#fff}.modal{box-shadow:none;max-height:none}}
      `}</style>
    </main>
  );
}
